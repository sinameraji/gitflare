...
|---|---|---|---|
| `RepoDO` | `REPO` | `POST /sync`, `GET /state` | `ref:<ref>` → `{ref, sha, syncedAt}` |
...
├── sync/                git-sync.ts (GitHub→Artifacts) · memfs.ts
├── artifacts/           refs.ts (listServerRefs) · content.ts (clone/tree/blob helpers)
├── github/webhook.ts    HMAC verification
├── deploy/              yaml.ts (subset parser) · workflow.ts (deploy.yml) · cf-deploy.ts (Scripts/Pages/D1 APIs)
