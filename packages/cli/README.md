...
- `gitflare ci import [--write]` — translate `.github/workflows/*.yml` into `.gitflare/ci.yml`, with a report of everything that didn't map (third-party actions, secrets, `if:`, matrix, non-Linux runners).
...
- `gitflare sync tags` / `gitflare sync issues` — one-time imports: existing tags into Artifacts; issues, PRs, comments, releases into the read-only metadata mirror (both then stay current via the webhook).
