...
- `gitflare ci import [--write]` — translate your existing `.github/workflows/*.yml` into a `.gitflare/ci.yml`: `run:` steps carry over (with `working-directory`, step `env`, `${{ github.sha }}`-style expressions mapped to `$GITFLARE_*`), `wrangler deploy` / `wrangler pages deploy` become `cloudflare/deploy` steps, `checkout`/`setup-node`/`setup-python`/`cache`/artifact actions are dropped as unnecessary, `pnpm/action-setup` becomes an install — and everything that has no equivalent (`uses:` of third-party actions, secrets, `if:`, matrix, `$GITHUB_OUTPUT`, non-Linux runners, non-push triggers) is listed with a reason. Dry run by default; the emitted file parses with GitFlare's own `ci.yml` parser.
