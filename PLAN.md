...
**Where to pick up (2026-08-19, night).** M0–M9 done; M10 is well underway. Shipped today after M9 (npm **0.6.0**, kimiflare Worker redeployed from it): Stage 2 Pages + D1 paths live-validated (two real bugs fixed: push deploys read the wrong branch; Pages asset keys had to be 32-hex); commit log (Artifacts `log()`), tag sync + `sync tags` backfill, GitHub health row, the read-only issues/PR/releases mirror (`MetaDO`), and `gitflare ci import`. **Still in M10**: M9 soak items (auth/backoff, `stalled`, a real outage), Access apps/policies live run, CI watchdog + cancel soak, reverse *tag* sync + tag/branch deletes, blame, LFS, per-job sandbox isolation, Pages build-artifact handover, R2 build cache, Browser Run, `gitflare sandbox`, a CI secrets store, CLI `resync/detach/secrets/logs`. Stages 4–6 are **not scheduled** (§4). npm publishes via OIDC — judge releases by `npm view gitflare version`, not the npm web page. Roadmap stages ≠ npm versions (§4 note).
...
| M10 | Backlog: Stage 3 remainder + Stage 1 completeness + validation debt | 📋 backlog (unordered) | Re-homed from M8's deferrals: per-job sandbox isolation, Pages build-artifact handover, R2 build cache keyed on lockfile hash, Browser Run for E2E, ~~GitHub Actions importer~~ (2026-08-19: `gitflare ci import` — dry-run translator + "couldn't translate, here's why" report; emitted YAML round-trips through the Worker parser), per-job commit-status contexts, `gitflare sandbox`; a CI secrets store (the importer keeps hitting `${{ secrets.* }}`). From M9: tag + branch-delete sync. From §4 Stage 1: ~~read-only issues/PR mirror~~ (2026-08-19: MetaDO — webhook-fed, alarm-driven GitHub-API import; issues/PRs/comments/reviews/releases with link-outs), ~~commit log~~ (2026-08-19, via the Artifacts binding's `log()`), blame, ~~tag list~~ (2026-08-19: tags sync on push + `gitflare sync tags` backfill; reverse tag sync and tag deletes still not propagated), ~~release list~~ (2026-08-19), ~~GitHub health row~~ (2026-08-19), LFS; CLI `resync`/`detach`/`secrets`/`logs`. Validation debt: ~~Pages Direct Upload, D1 migrations~~ (done 2026-08-19), Access apps/policies, CI watchdog + cancel soak, M9 auth/backoff path. Each item is independent — pick by demand. |
