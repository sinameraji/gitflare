...
Per version:
- **v0.1** — `init`, `status`, `resync`, `detach`, `secrets`, `logs`. *(Status 2026-08-19: `init` + `status` shipped; `resync`/`detach`/`secrets`/`logs` deferred → M10. `access enable|disable` was added here instead.)*
- **v0.2** — `deploy`, `deploys list`, `rollback`. *(Shipped as `deploy enable|disable|run|list|rollback`.)*
- **v0.3** — `run <workflow>`, `sandbox` (spin up a Sandbox with current branch, drop into a shell with optional port forwards). *(Shipped as `ci enable|disable|run|list|cancel` — `run` takes no workflow name, it runs the repo's `ci.yml` at the current Artifacts HEAD; `sandbox` deferred → M10. M9 adds `sync enable|disable|now|status` and `remote add|remove`.)*
...
- v0.3 adds: the account-level **Containers** permission (needed the moment `gitflare ci enable` puts a `[[containers]]` block in the Worker config — exact permission name to be confirmed on the first live run; `ci enable` warns about this up front). Later in v0.3: `R2: Edit` (specifically *Workers R2 Storage*, for build cache); `D1: Edit` (for CI run history)
...
**Where to pick up (2026-08-19).** M0–M8 are done (v0.3.1 tagged; npm publishing moved to OIDC trusted publishing — confirm `npm view gitflare version` ≥ 0.3.2 before trusting the npm page). Next is **M9** — the missing half of the founding vision (§5): push to the mirror while GitHub is down, CI/CD runs, GitHub catches up when it's back. Everything M8 deferred now lives in **M10**. Honest version-level state vs §4: v0.1 ≈ 70 % (metadata mirror + log/blame/tags never built — see the note under §4 v0.1), v0.2 ≈ 85 % (Pages + D1 paths not yet run live), v0.3 ≈ 50 %, v0.4+ not started; ≈ 20 % of the v1.0 vision overall, ≈ ⅔ of the solo-developer arc.
